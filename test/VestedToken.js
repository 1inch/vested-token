...
describe("VestedToken", async function () {
    ...
    it("should register and deregister vestings, update balances correctly, and handle vesting claims as expected", async function () {
        ...
        expect(
            await this.VestedToken.balanceOf(
                "0x4942b20750163675DDf004476FFE46626652dfD0"
            )
        ).to.be.bignumber.equal(ether("18750000"));
        expect(
            await this.VestedToken.balanceOf(
                "0x4Dc06c025Cb97d6Dc4A17183187e28D297B83C7B"
            )
        ).to.be.bignumber.equal(ether("13926250"));
        expectEvent(
            await this.VestedToken.deregisterVestings([
                "0x32757Da4Bd73a35F05806Ee91CDB4B1746e63a45",
            ]),
            "VestingDeregistered",
            {
        ...
        ).to.be.bignumber.equal(ether("11073750"));
        ...
        const vesting = await IStepVesting.at(
            "0x0ac6ff4dBdE9cEeDC6d4c08B14ceD9FF19A013aB"
        );
        expect(
            await this.VestedToken.balanceOf(
        ...
        expectEvent(
            await this.VestedToken.updateBalances([
                "0x0ac6ff4dBdE9cEeDC6d4c08B14ceD9FF19A013aB",
            ]),
            "Transfer",
            {
